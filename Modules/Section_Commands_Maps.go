...
var View = map[string]func(){
	"uuids": func() {
		cols := []string{"UUID", "IP"}
		var rows [][]string
		for i := 0; i < len(UUIDCAPS); i++ {
			if len(UUIDCAPS) == len(UUIDHOSTS) {
		...
		for i := 0; i < len(MACS); i++ {
			if len(MACS) == len(IPS) {
				...
				if manufac == "Google, Inc." {
					...
					// try and make a connection as a common port for a cast.
					cl := http.Client{
						Timeout: time.Second * 2,
					}
					host := IPS[i]
					mac := MACS[i]
					go func(host, mac string) {
	...
	"routers": func() {
		...
		for i := 0; i < len(MACS); i++ {
			if len(MACS) == len(IPS) {
				manufac := GrabManufac(MACS[i])
				if manufac == "ARRIS Group, Inc." || IPS[i] == "10.0.0.1" {
					rows = append(rows, []string{
						IPS[i],
						MACS[i],
						"ARRIS GROUP | Standard router",
					})
				}
			}
		}
	...
	"amazon": func() {
		...
		for i := 0; i < len(MACS); i++ {
			if len(MACS) == len(IPS) {
				manufac := GrabManufac(MACS[i])
				if manufac == "Amazon Technologies Inc." {
					rows = append(rows, []string{
						IPS[i],
						MACS[i],
...
var Enumerate = map[string]func(){
	"appleinfo": func() {
		if TargetMain != "" {
			...
			f, x := http.Get(fmt.Sprintf(ApplePathMap["binfile"], TargetMain))
			if x != nil {
				log.Fatal(x)
			}
			defer f.Body.Close()
			if f.StatusCode == 200 {
				scanner := bufio.NewScanner(f.Body)
				var binf []string
				for scanner.Scan() {
					binf = append(binf, scanner.Text())
				}
				newf := WriteToFile("info", binf)
				DumpFile(newf)
			}
		}
	...
	}, // this function will always take a input from the user and allow them to basically remotely controll a roku TV
	"roku-activeapp": func() {
		if TargetMain != "" {
	...
	// Router enumeration and information
	"routerXF-DI":           func() {}, // Device information
	"routerXF-SL":           func() {}, // Service list
	"routerXF-L3Forwarding": func() {}, // Layer 3 forwarding on SCPD
	//GDDEVDESC
	"routerXF-GDD": func() {
		if TargetMain != "" {
			ur := RoutesARRIS["GDDEVDESC"]
			url := fmt.Sprintf(ur, TargetMain)
			ostream, x := http.Get(url)
			if x != nil {
				ErrorHandler[10]()
				fmt.Print(x)
				return
			}
			defer ostream.Body.Close()
			if ostream.StatusCode == 200 {
				bv, _ := ioutil.ReadAll(ostream.Body)
				var DEVDESC Root
				xml.Unmarshal(bv, &DEVDESC)
				rows := [][]string{
					{"Device Type", DEVDESC.Device.DeviceType},
					{"Device Name", DEVDESC.Device.FriendlyName},
					{"Device Manufac", DEVDESC.Device.Manufacturer},
					{"Device Desc", DEVDESC.Device.ModelDescription},
					{"Device Model Name", DEVDESC.Device.ModelName},
					{"Device Model", DEVDESC.Device.ModelNumber},
					{"Device Serial Number", DEVDESC.Device.SerialNumber},
					{"Device UDN", DEVDESC.Device.UDN},
					{"Device UPC", DEVDESC.Device.UPC},
					{"Control URL", DEVDESC.Device.ServiceList.Service.ControlURL},
					{"event SubURL", DEVDESC.Device.ServiceList.Service.EventSubURL},
					{"ServiceID", DEVDESC.Device.ServiceList.Service.ServiceId},
					{"Service Type", DEVDESC.Device.ServiceList.Service.ServiceType},
				}
				DrawVerticle(rows)
			}
		} else {
	...
	"samsungCMS-off": func() {
		if TargetMain != "" {
			sprinturl := PathsCMS["powerdown"]
			url := fmt.Sprintf(sprinturl, TargetMain)
			f, x := http.Get(url)
			if x != nil {
				ErrorHandler[10]()
				fmt.Print(x)
	...
	}, // Exploit and turn the samsung smart TV off ( version unknown, not tested)
	// Enumerate Amazon Technologies devices
	"firestick-devinfo": func() {
		if TargetMain != "" {
			RequestUUIDInformation(TargetMain)
		}
	},
}
